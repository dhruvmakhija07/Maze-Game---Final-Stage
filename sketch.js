...
function setup(){
  ...
  var energy2position = [
    ...
    {x: 400, y: 160, image: applesprite},
    {x: 155, y: 543, image: applesprite}
  ];
...
function draw(){
  ...
  strokeWeight(2);
  stroke(51);
  text("AdventureScape Maze",40,40);
  ...
  strokeWeight(2);
  stroke(51);
  text("Lives ❤️: "+lives,575,50);
  
  textSize(14);
  fill(94, 106, 10);
  strokeWeight(2);
  stroke(51);
  text("Player: "+user,40,672);

  textSize(10);
  fill(94, 106, 10);
  strokeWeight(2);
  stroke(51);
  text("Use '↑' key to move up.",545,660) 

  textSize(10);
  fill(94, 106, 10);
  strokeWeight(2);
  stroke(51);
  text("Use '↓' key to move down.",545,675) 

  textSize(10);
  fill(94, 106, 10);
  strokeWeight(2);
  stroke(51);
  text("Use '→' key to move right.",400,675)

  textSize(10);
  fill(94, 106, 10);
  strokeWeight(2);
  stroke(51);
  text("Use '←' key to move left.",400,660)

  if(player.isTouching(maze1sprite) || player.isTouching(maze2sprite) 
  || player.isTouching(maze3sprite) || player.isTouching(maze4sprite)
  || player.isTouching(maze5sprite) || player.isTouching(maze6sprite)
  || player.isTouching(maze7sprite) || player.isTouching(maze8sprite)
  || player.isTouching(maze9sprite) || player.isTouching(maze10sprite)
  || player.isTouching(maze11sprite) || player.isTouching(maze12sprite)
  || player.isTouching(maze13sprite) || player.isTouching(maze14sprite)
  || player.isTouching(maze15sprite) || player.isTouching(maze16sprite)
  || player.isTouching(maze17sprite) || player.isTouching(maze18sprite)
  || player.isTouching(maze19sprite) || player.isTouching(maze20sprite)
  || player.isTouching(maze21sprite) || player.isTouching(maze22sprite)
  || player.isTouching(maze23sprite) || player.isTouching(maze24sprite)
  || player.isTouching(maze25sprite) || player.isTouching(maze26sprite)
  || player.isTouching(maze27sprite) || player.isTouching(maze28sprite)
  || player.isTouching(maze29sprite) || player.isTouching(maze30sprite)
  || player.isTouching(maze31sprite)){
...
function rules(){
  swal({
    title: 'Welcome to AdventureScape Maze!',
    text: "Rules: Don't collide with the maze or you will lose your lives, Collect fruits for energy or you'll die, collect rewards to increase your score, and reach the finish line to win the game!",
...
function gameWon(){
  swal({
    ...
    imageUrl:
      "https://hotemoji.com/images/dl/g/party-popper-emoji-by-google.png",
    imageSize: "100x100",
    confirmButtonText: "Thanks For Playing"
  },
